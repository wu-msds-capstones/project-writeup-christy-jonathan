...
when formulating housing strategies.
...
Our previous analysis left us with a subset of combinations to use for fitting a linear model. The list was small enough that we could fit a 
linear regression model to all combinations and evaluate the best fit.  Finding models with a good fit allowed us to note the features associated with
this combination and to use this to compare against our random forest model.  While looping through our feature combinations, we also employed the 
Anderson-Darling test to test that the residuals had a normal distribution so that we could evaluate the feasibility of the fit.  If the p-value of the 
Anderson-Darling test was < .05, we call this not normally distributed for that set of combinations.

##### 5. Building a Random Forest Model

Beginning again with our 
